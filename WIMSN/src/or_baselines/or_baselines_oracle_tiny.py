...
if __name__ == "__main__":
    ...
    RESULT_DIR = "output_tiny"
    os.makedirs(RESULT_DIR, exist_ok=True)
    os.makedirs(os.path.join(RESULT_DIR, "or_baselines"), exist_ok=True)
    output_dir = os.path.join(RESULT_DIR, "or_baselines")
    ...
    for task in task_list:
        for mode in mode_list:
            for policy in ['ss', 'bs']:
                ...
                initial_X = initials.loc[(initials['task'] == task) & (initials['mode'] == mode) & (initials['policy'] == policy)]
                if policy == 'ss':
                    initial_X = [initial_X['S'].values[0]] * sku_count + [initial_X['s'].values[0]] * sku_count
                elif policy == 'bs':
                    initial_X = [initial_X['basestock'].values[0]] * sku_count
                initial_X = np.array(initial_X)
                info = search_different_Ss(env, policy=policy, initial_X=initial_X)
                info.update(dict(task=task, mode=mode, policy=policy))
                print(info)
                record.append(info)

                pd.DataFrame(record).to_csv(os.path.join(output_dir, 'tmp_different_parameters.csv'))

    record = pd.DataFrame(record)
    record.to_csv(os.path.join(output_dir, 'different_parameters.csv'))
