...
class EvaluateRunner:
    ...
    def run(self, lbda_index=None, test_mode=False, 
        ...
        self.reset(test_mode=test_mode, storage_capacity=storage_capacity[0])
        ...
        while True:
            ...
            if self.t == 50:
                for parent_conn in self.parent_conns:
                    parent_conn.send(("set_storage_capacity", storage_capacity[1]))
            ...
            if self.args.mac == "mappo_mac":
                ...
                mac_output = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, 
                    lbda_indices=lbda_indices, bs=envs_not_terminated, test_mode=test_mode)
            ...
            record_rewards.append(post_transition_data['reward'])

        if not test_mode:
            self.t_env += self.env_steps_this_run

        record_rewards = np.array(record_rewards).squeeze(-1)
